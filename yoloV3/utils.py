...
imagePath = 'C:/Users/basro/Desktop/python_projects/imageCombination/ProjectD-Gerdwin/images/combineImgs/'
...
def draw_boxes(img_names, boxes_dicts, class_names, model_size):
    ...
    for num, img_name, boxes_dict in zip(range(len(img_names)), img_names, boxes_dicts):
        ...
        font = ImageFont.truetype(font='./yoloV3/data/fonts/futur.ttf',
            ...
            if np.size(boxes) != 0:
                ...
                for box in boxes:
                    ...
                    for t in np.linspace(0, 1, thickness):
                        ...
                        newcoords = xy.copy()
                        newcoords.append(class_names[cls])

                        if (newcoords[4] == 'person' and person == True):
                            coordsList.append(newcoords)
                            person = False
                        if (newcoords[4] == 'body' and body == True):
                            coordsList.append(newcoords)
                            body = False
                        if (newcoords[4] == 'legs' and legs == True):
                            coordsList.append(newcoords)
                            legs = False
                        if (newcoords[4] == 'head' and head == True):
                            coordsList.append(newcoords)
                            head = False
        ...
        rgb_img.save(imagePath + str(num + 1) + '.jpg')


        endCoords.append(coordsList)
    return endCoords
